...
// Debugger function, counts elements in fs tree
void count_fstree(void *obj) {
    if (!obj) {
        fprintf(stderr, "Error: cannot count Fstree, obj is NULL.\n");
    } else {
        if (!mark_one(obj)) { //check obj already marked, proceed if not.
        Fstree *tree = obj;
        mark_one(tree->name);
...
void sig_handler_tree(int signum) {
    if (signum == SIGUSR1) {
